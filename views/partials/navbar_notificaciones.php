...
<style>
  ...
  .notif-item       { display: flex; align-items: flex-start; gap: 10px; padding: 12px 16px; border-bottom: 1px solid #f0f0f0; transition: background 0.15s; position: relative; }
          ...
          <div class="notif-item" id="notif-<?= $alerta['id_alerta'] ?>">
            <div class="notif-body">
              ...
              <div class="d-flex justify-content-between align-items-start">
                <p class="notif-producto">
                  <?= htmlspecialchars($alerta['nombre_prod']) ?>
                </p>
                <button
                  onclick="cambiarEstadoNotif(<?= $alerta['id_alerta'] ?>)"
                  class="btn btn-outline-success btn-sm p-0 px-1"
              ...
              <p class="notif-detalle">
                Stock actual: <span><?= $alerta['stock_actual'] ?></span>
              </p>
 
              <!-- Botón proveedor — solo si el JOIN devolvió datos -->
              <?php if (!empty($alerta['nombre_empresa'])): ?>
                <button
                  class="btn btn-dark btn-sm mt-2 w-100"
                  style="font-size: 10px;"
                  onclick="mostrarInfoProveedor(
                    '<?= addslashes($alerta['nombre_empresa']) ?>',
                    '<?= addslashes($alerta['contacto_nombre'] ?? 'Sin contacto') ?>',
                    '<?= $alerta['telefono'] ?? 'N/A' ?>'
                  )">
                  📦 Ver Proveedor
                </button>
              <?php endif; ?>   <!-- ✅ endif cerrado correctamente -->
...
function cambiarEstadoNotif(id) {
  ...
  fetch('/gualo_electronic/actions/desactivar_notificacion.php?id=' + id)
    .then(r => r.json())
    .then(data => {
      if (data.ok) {
